...
public class FileConvertion {
	...
	public void xmlToXSDConversion() {
		...
		try {
			in = new Scanner(new File(
					PESEditor.fileLocation + "/" + PESEditor.projName + "/outputgraphxmlforxsd.xml")); // outputgraphxmlforxsd
	...
	public void modifyXmlOutputForXSD() {
		...
		try {
			in = new Scanner(new File(
					PESEditor.fileLocation + "/" + PESEditor.projName + "/graphxmluniformity.xml"));// graphxml
																												// //graphxmluniformity
		} catch (FileNotFoundException e) {
	...
	public void addConstraintToSESStructure(String[] sesNodesInPath, String constraint) {
		...
		try {
			f0 = new PrintWriter(
					new FileWriter(PESEditor.fileLocation + "/" + PESEditor.projName + "/testcon.xml"));// outputgraphxmlforxsdvar
	...
	public void placeAssertInRightPosition() {
		...
		try {
			// if (secondtime < 2) {
			in = new Scanner(
					new File(PESEditor.fileLocation + "/" + PESEditor.projName + "/xsdfromxml.xsd"));
			// secondtime++;
			// } else
