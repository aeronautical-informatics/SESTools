...
public class TypeInfoWriter extends DefaultHandler {
	...
	public void startElement(String uri, String localName, String qname, Attributes attributes) throws SAXException {
		...
		if (attributes == null) {
			...
			for (int i = 0; i < length; i++) {
				...
				Console.addConsoleOutput(fTypeInfoProvider != null && fTypeInfoProvider.isIdAttribute(i) ? "\"true\"" : "\"false\"");
				Console.addConsoleOutput(",");
				Console.addConsoleOutput("specified=");
				Console.addConsoleOutput(fTypeInfoProvider == null || fTypeInfoProvider.isSpecified(i) ? "\"true\"" : "\"false\"");
				Console.addConsoleOutput("}");
			}
			Console.addConsoleOutput("}");
		}
	...
	public static void validateXML() {
		...
		schemaLanguage = XSD11_SCHEMA_LANGUAGE;
			...
			if (PESEditor.sesValidationControl == 1) {
				instances.add(PESEditor.fileLocation+"/"+PESEditor.projName+"/xmlforxsd.xml");
				PESEditor.sesValidationControl = 0;
			} else {
				String rootNodeName = PESEditor.jtreeTograph.rootNodeName();
				instances.add(PESEditor.fileLocation+"/"+PESEditor.projName+"/" + rootNodeName + ".xml");
			}
			
		
		/*
		//----------------------------------------------------------------------------start of argument matching and assignment-------------------------------------
		//-xsd11 -i xsd11_datatype_test.xml
		//-----------------------------------------
		for (int i = 0; i < argv.length; ++i) {
			...
			if (arg.startsWith("-")) {
				...
				if (option.equals("l")) {
					...
					if (++i == argv.length) {
						Console.addConsoleOutputln("error: Missing argument to -l option.");
					} else {
				...
				if (option.equals("p")) {
					...
					try {
						...
						try {
							...
							Console.addConsoleOutputln("warning: Features and properties not supported on SAX1 parsers.");
						} catch (Exception ex) {
							parser = null;
							Console.addConsoleOutputln("error: Unable to instantiate parser (" + parserName + ")");
				...
				Console.addConsoleOutputln("error: unknown option (" + option + ").");
				continue;
			}
		}
...
*/
		...
		try {
			...
			try {
				...
				Console.addConsoleOutput("warning: SchemaFactory does not support feature (" + VALIDATE_ANNOTATIONS_ID + ")");
			}
			try {
			...
			try {
				...
				Console.addConsoleOutput("warning: Validator does not recognize feature (" + VALIDATE_ANNOTATIONS_ID + ")");
			} catch (SAXNotSupportedException e) {
				Console.addConsoleOutput("warning: Validator does not support feature (" + VALIDATE_ANNOTATIONS_ID + ")");
			}
			try {
