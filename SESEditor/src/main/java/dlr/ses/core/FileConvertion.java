...
public class FileConvertion {
			...
			if (line.startsWith("<?")) { // have to solve space problem for this line
				...
				if (line.endsWith("/>")) {
					...
					if (result.endsWith("Var")) {
						...
						String resultCon = backConstraints.replaceAll("[<>]", "");// to keep the space backConstraints
																					// is used here otherwise else and
																					// true() will be elsetrue();
						String nonconresult = resultCon.replace("Con/", "");// with Con also / added here then Con/ for
					...
					if (result.endsWith("Dec")) {
						...
						if (entiyAfterMAsp == 1) {
							mod = "<xs:element name=\"" + result + "\" minOccurs=\"0\" maxOccurs=\"unbounded\">";
							f0.println(mod);
							f0.println("<xs:complexType>");
	...
	public void modifyXmlOutputForXSD() {
		...
		Scanner in = null;
		try {
			in = new Scanner(new File(SESEditor.fileLocation + "/" + SESEditor.projName + "/graphxmluniformity.xml"));// graphxml
																														// //graphxmluniformity
		} catch (FileNotFoundException e) {
	...
	public void addConstraintToSESStructure(String[] sesNodesInPath, String constraint) {
		...
		PrintWriter f0 = null;
		try {
			f0 = new PrintWriter(new FileWriter(SESEditor.fileLocation + "/" + SESEditor.projName + "/testcon.xml"));// outputgraphxmlforxsdvar
		...
		while (in.hasNext()) {
			...
			if (line.startsWith("<?")) {
				...
				if (count < len) {
					...
					if (count == len) {
						f0.println(
								"<" + splited[0] + " " + splited[1] + " " + "constraint=\"" + constraint + "\" " + ">");
					} else {
						f0.println(line);
					}
				} else {
	...
	public void fixingSequenceProblem(String[] stringArrayRev) {
		...
		try {
			f0 = new PrintWriter(
					new FileWriter(SESEditor.fileLocation + "/" + SESEditor.projName + "/outputgraphxmlforxsdseq.xml"));
			// System.out.println("output file generated");
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
	...
	public void copyfixingSequenceFileToExistingOne() {
		...
		try {
			in = new Scanner(
					new File(SESEditor.fileLocation + "/" + SESEditor.projName + "/outputgraphxmlforxsdseq.xml"));
			// System.out.println("my read complete");
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public void copyxsdfromxmlToRootNodeNameXSD() {
		...
		try {
			f0 = new PrintWriter(
					new FileWriter(SESEditor.fileLocation + "/" + SESEditor.projName + "/" + rootNodeName + ".xsd"));
			// System.out.println("output file generated");
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
	...
	public void createSES() {
		...
		PrintWriter f0 = null;
		try {
			f0 = new PrintWriter(new FileWriter(SESEditor.fileLocation + "/" + SESEditor.projName + "/ses.xsd"));
			// System.out.println("output file generated");
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
