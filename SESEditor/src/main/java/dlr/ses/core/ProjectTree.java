...
public class ProjectTree extends JPanel implements MouseListener {
	...
	File ssdFileProject = new File(SESEditor.projName + "/" + JtreeToGraph.newFileName + "Project.xml");
	...
	public ProjectTree() {
		...
		if (ssdFileProject.exists()) {
			...
			XmlJTree myTree = new XmlJTree(SESEditor.projName + "/" + JtreeToGraph.newFileName + "Project.xml");
		...
		if (currentSelection != null) {
			...
			if (parent != null) {
				...
				if (currentNode.toString().equals("MainModule") || currentNode.toString().equals("AddedModule")
						|| currentNode.toString().equals(SESEditor.projName + ".xml")) {
					toolkit.beep();
				} else {
